...
int pack_rom(unsigned int chiptype, const char *loader_filename, int majver, int minver, int subver, const char *image_filename, const char *outfile)
{
    ...
    // Set rom_header.code based on the chiptype
    if(chiptype == 0x50) {
    ...
    // [ADDED for RK3566/RK3568]
    else if (chiptype == 0x33353636) { // ASCII "3566"
        rom_header.code = 0x01070000; // or a suitable value for RK3566
    } else if (chiptype == 0x33353638) { // ASCII "3568"
        rom_header.code = 0x01070000; // or a suitable value for RK3568
    }
...
void usage(const char *appname)
{
    ...
    printf("USAGE:\n"
           ...
           "Example:\n"
           "%s -rk29 Loader.bin 1 0 23 rawimage.img rkimage.img    \tRK29 board\n"
           ...
           // [ADDED for RK3566/RK3568]
           "%s -rk3566 Loader.bin 1 0 0 rawimage.img rkimage.img   \tRK3566 board\n"
           "%s -rk3568 Loader.bin 1 0 0 rawimage.img rkimage.img   \tRK3568 board\n"
           "\n\n"
           "Options:\n"
           "[chiptype]:\n"
           "\t-rk29\n"
...
int main(int argc, char **argv)
{
    ...
    if (argc == 8)
    {
        ...
        // [ADDED for RK3566/RK3568]
        else if (strcmp(argv[1], "-rk3566") == 0)
        {
            ret = pack_rom(0x33353636, argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), argv[6], argv[7]);
        }
        else if (strcmp(argv[1], "-rk3568") == 0)
        {
            ret = pack_rom(0x33353638, argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), argv[6], argv[7]);
        }
        else
        {
            usage(argv[0]);
